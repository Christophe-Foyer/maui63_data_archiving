...
def apply_autodistill_segmentation(dataset_path):
    ...
    try:
        ...
        print(
            "    Please ensure 'transformers', 'torch', 'scikit-image', 'pycocotools' are installed."
        )
        return
    ...
    for ann_file in annotation_files:
        ...
        try:
            ...
            for img_id, anns in tqdm(
                img_to_anns.items(), desc=f"Segmenting {ann_file.name}"
            ):
            ...
            if modified_count > 0:
                print(
                    f"Updated {modified_count} annotations in {ann_file.name}. Saving..."
                )
                # Backup
...
def download_roboflow_dataset(
    api_key, projects, format="yolov8", apply_autodistill=False
):
    try:
        ...
        for proj_config in projects:
            ...
            try:
                ...
                dataset = project.version(version).download(
                    format, location=str(DATA_DIR / output_name)
                )
...
def main():
    parser = argparse.ArgumentParser(
        description="Download and optionally process ML datasets."
    )
    parser.add_argument(
        "--autodistill",
        action="store_true",
        help="Apply SAM-based auto-distillation (segmentation) to Roboflow datasets.",
    )
    ...
    roboflow_projects = [
        {
            "workspace": (path if isinstance(path, str) else path[0]).split("/")[0],
            "project": (path if isinstance(path, str) else path[0]).split("/")[1],
            "version": (1 if isinstance(path, str) else path[1]),
            "output_dir_name": (
                (path if isinstance(path, str) else path[0]).split("/")[1]
            ),
        }
        for path in [
            ...
            "panoptes/panoptes",
            ("karina-macosko-kynfu/sea-turtle-classification-iexit", 8),
            ("gabriel-esteves-dy2cw/sea-turtles-yia2e", 2),
            "charliebrian/redsea_stingray_detection_test-vh1si",
            "z-corrugated-iron-fire/dolphin-psql9",  # mediocre quality but the variation of images is good
            "seal-vzuwh/seal-fmc1j",
            "kspicy/seal-ekfsj",
            "kspicy/seal-alert",
        ]
    ]
    ...
    if api_key:
        download_roboflow_dataset(
            ...
            roboflow_projects,
            format="coco",
            apply_autodistill=args.autodistill,
        )
    else:
        print(
            "Skipping Roboflow download. Set 'ROBOFLOW_API_KEY' environment variable to download."
        )
        print("You can get a key from https://app.roboflow.com/")
        for p in roboflow_projects:
            print(
                f"Target Dataset: https://universe.roboflow.com/{p['workspace']}/{p['project']}"
            )
